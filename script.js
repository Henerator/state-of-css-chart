...
const scaleX = d3.scaleBand().domain(years).range([0, linesRectSize.width]).round(true);
...
const svg = d3.select('#chart-container').append('svg').attr('width', chartSize.width).attr('height', chartSize.height);
...
yearAxisGroups
  ...
  .attr('fill', colors.text)
  .text((year) => year)
  .attr('transform', `translate(0 ${linesRectSize.height + chartMargin.yearAxisText})`);
...
interactionLineGroups.append('path').classed('line-start', true).attr('d', drawLineStart);
interactionLineGroups.append('path').classed('line-end', true).attr('d', drawLineEnd);
...
interactionGroup
  .on('mouseenter', function (_event, rowItemData) {
    ...
    selectedRow.selectAll('g.lines').transition(getTransition()).attr('stroke-width', '8');
    selectedRow.selectAll('.circle').transition(getTransition()).attr('stroke-width', '4').attr('r', '21');
    selectedRow.selectAll('.circle-shadow').transition(getTransition()).attr('r', '23');

    const otherRows = dataGroups.filter((itemData) => itemData.name !== name);
    otherRows.transition(getTransition()).attr('opacity', '0.4');
    otherRows.selectAll('g.lines').transition(getTransition()).attr('stroke-width', '5');
    otherRows.selectAll('.circle').transition(getTransition()).attr('stroke-width', '0').attr('r', '0');
    otherRows.selectAll('.circle-shadow').transition(getTransition()).attr('r', '0');
  ...
  .on('mouseleave', function (_event, rowItemData) {
    ...
    const selectedRow = dataGroups.filter((itemData) => itemData.name === name);
    selectedRow.selectAll('g.lines').transition(getTransition()).attr('stroke-width', '5');
    selectedRow.selectAll('.circle').transition(getTransition()).attr('stroke-width', '3').attr('r', '18');
    selectedRow.selectAll('.circle-shadow').transition(getTransition()).attr('r', '19');

    const otherRows = dataGroups.filter((itemData) => itemData.name !== name);
    otherRows.transition(getTransition()).attr('opacity', '1');
    otherRows.selectAll('.circle').transition(getTransition()).attr('stroke-width', '3').attr('r', '18');
    otherRows.selectAll('.circle-shadow').transition(getTransition()).attr('r', '19');
...
function onResize() {
  ...
  yearAxisGroups.transition(getTransition()).attr('transform', transformYearAxisGroups);

  lineGroups.selectAll('path.line').transition(getTransition()).attr('d', drawLine);
  lineGroups.selectAll('path.line-start').transition(getTransition()).attr('d', drawLineStart);
  lineGroups.selectAll('path.line-end').transition(getTransition()).attr('d', drawLineEnd);

  interactionLineGroups.selectAll('path.line').transition(getTransition()).attr('d', drawLine);
  interactionLineGroups.selectAll('path.line-start').transition(getTransition()).attr('d', drawLineStart);
  interactionLineGroups.selectAll('path.line-end').transition(getTransition()).attr('d', drawLineEnd);

  circleGroups.transition(getTransition()).attr('transform', transformCircleGroups);

  textEndGroups.transition(getTransition()).attr('transform', transformTextEndGroups);
}
...
function updateDataType(type) {
  selectedType = type;
  const lineGroupsToUpdate = dataGroups.select('g.lines').datum((item) => item[selectedType]);
  lineGroupsToUpdate.select('path.line').transition(getTransition()).attr('d', drawLine);
  lineGroupsToUpdate.select('path.line-start').transition(getTransition()).attr('d', drawLineStart);
  lineGroupsToUpdate.select('path.line-end').transition(getTransition()).attr('d', drawLineEnd);

  const circleGroupsToUpdate = dataGroups.selectAll('g.circle-group').data((item) => item[selectedType]);
  circleGroupsToUpdate.transition(getTransition()).attr('transform', transformCircleGroups);

  textStartGroups.transition(getTransition()).attr('transform', transformTextStartGroups);
  textEndGroups.transition(getTransition()).attr('transform', transformTextEndGroups);

  const interactionLineGroupsToUpdate = dataGroups.select('g.interaction-line').datum((item) => item[selectedType]);
  interactionLineGroupsToUpdate.select('path.line').transition(getTransition()).attr('d', drawLine);
  interactionLineGroupsToUpdate.select('path.line-start').transition(getTransition()).attr('d', drawLineStart);
  interactionLineGroupsToUpdate.select('path.line-end').transition(getTransition()).attr('d', drawLineEnd);
}
...
buttons.on('click', ({ target }) => {
  ...
  buttons.each(function () {
    const button = this;
    button.dataset.type === selectedType ? button.classList.add('active') : button.classList.remove('active');
  });
